...
KRAKEN_COMPUTED_INTERVALS = [1, 5, 15, 60, 720, 1440]
...
def __load_historical_kraken_exports():
...
def __determine_optimal_source_interval(desired_interval: int) -> int:
    ...
    computed_intervals = sorted([1, 5, 15, 60, 720, 1440])
...
def calc_spot_price_by_minute(num_minutes: int = 1440) -> pd.DataFrame:
    ...
    # source_df['date'] = source_df['time'].apply(datetime.utcfromtimestamp)
    ...
    oldest_ts_series = source_df.groupby('interval_group')[['time', 'open', 'low']].min()
    latest_ts_series = source_df.groupby('interval_group')[['close', 'high']].max()
    ...
    result_df.columns = list(source_df.columns)[:-1]
    return result_df


# __load_historical_kraken_exports()
df_720 = calc_spot_price_by_minute(720)
print(df_720.head(5))
